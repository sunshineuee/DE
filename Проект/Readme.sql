...
имеет три ключевых поля UNIQUE(warehouse_id,nomenclature_id,series_id) и почему-то не ругается на дубли по набору ключевых полей...
...
изменение таблицы operations изменяет текущее значение stock_balances.count_move по трем соответствующим ключевым полям 
		на величину operations.count_move со знаком + или -, взависимости от значения (operations.document_type_id.direction)
		если direction = 1 - остатки увеличиваются (документ считается приходным для склада)
		если direction = 0 - остатки уменьшается (документ считается расходным для склада)
...
Отчета два:
1. divergence_of_stock_balances показывает
